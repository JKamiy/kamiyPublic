...
echo "$lrr" | sed -e "s/RRAID/$1/g" > /jd/scripts/10_redrain.js

echo "红包雨RRA替换成$1完成"

# 删除旧的定时任务
sed -i '/10_redrain.js/d' /jd/config/crontab.list

echo "删除旧的10_redrain定时任务完成"

# 添加今天的定时任务
echo "1 8-23$(date "+ %-d %-m") * jtask 10_redrain.js now"  >>  /jd/config/crontab.list

echo "添加当天的10_redrain红包雨定时任务完成"

echo "顺便白嫖一下红包雨"

jtask 10_redrain now > /dev/null 
